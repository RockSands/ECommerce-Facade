...
import com.ec.common.utils.NumberUtils;
import com.ec.user.mapper.UserMapper;
import com.ec.user.pojo.User;
import com.ec.user.utils.CodecUtils;
...
public class UserService {
    ...
    public void register(User user, String code) {
        ...
        if (!StringUtils.equals(code, value)) {
            //验证码不匹配
            throw new ECException(ExceptionEnum.VERIFY_CODE_NOT_MATCHING);
        }
    ...
    public User queryUser(String username, String password) {
        ...
        if (user == null) {
            throw new ECException(ExceptionEnum.USER_NOT_EXIST);
        }

        //检验密码是否正确
        if (!StringUtils.equals(CodecUtils.md5Hex(password, user.getSalt()), user.getPassword())) {
            //密码不正确
            throw new ECException(ExceptionEnum.PASSWORD_NOT_MATCHING);
        }
