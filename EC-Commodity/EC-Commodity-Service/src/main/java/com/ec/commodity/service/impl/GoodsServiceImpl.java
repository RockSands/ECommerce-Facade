...
import com.ec.commodity.mapper.BrandMapper;
import com.ec.commodity.mapper.CategoryMapper;
import com.ec.commodity.mapper.SkuMapper;
import com.ec.commodity.mapper.SpuDetailMapper;
import com.ec.commodity.mapper.SpuMapper;
import com.ec.commodity.mapper.StockMapper;
...
public class GoodsServiceImpl implements GoodsService {
	...
	public void deleteGoodsBySpuId(Long spuId) {
		...
		if (count == 0) {
			throw new ECException(ExceptionEnum.DELETE_GOODS_ERROR);
		}
	...
	public void addGoods(Spu spu) {
		...
		if (count != 1) {
			throw new ECException(ExceptionEnum.GOODS_SAVE_ERROR);
		}
		// 插入spuDetail数据
	...
	public void updateGoods(Spu spu) {
		...
		int count = spuMapper.updateByPrimaryKeySelective(spu);
		if (count == 0) {
			throw new ECException(ExceptionEnum.GOODS_UPDATE_ERROR);
		}
	...
	public void handleSaleable(Spu spu) {
		...
		int count = spuMapper.updateByPrimaryKeySelective(spu);
		if (count != 1) {
			throw new ECException(ExceptionEnum.UPDATE_SALEABLE_ERROR);
		}
	}
	...
	private void fillStock(List<Long> ids, List<Sku> skus) {
		...
		if (CollectionUtils.isEmpty(stocks)) {
			throw new ECException(ExceptionEnum.STOCK_NOT_FOUND);
		}
	...
	private void handleCategoryAndBrand(List<Spu> spuList) {
		for (Spu spu : spuList) {
			// 根据spu中的分类ids查询分类名
			List<String> nameList = categoryMapper
					.selectByIdList(Arrays.asList(spu.getCid1(), spu.getCid2(), spu.getCid3())).stream()
